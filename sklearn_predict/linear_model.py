...
class LinearSVMSQL:
    ...
    def export(self):
        ...
        if len(self.model.coef_.shape) == 1:
            query = " + ".join(
                [
                    "(({}) * (`{}`))".format(coef, nm)
                    for coef, nm in zip(self.model.coef_, self.feature_names)
                ]
            )
            ...
            for n_class in range(n_classes):
                query = " + ".join(
                    [
                        "(({}) * (`{}`))".format(coef, nm)
                        for coef, nm in zip(
                            self.model.coef_[n_class, :], self.feature_names
                        )
                    ]
                )
    ...
    def to_sql(
        self, tbl_name, pred_prefix="pred", pred_target=None, pred_name="prediction"
    ):
        """
        ...
        select_query = ",".join(
            ["{} > 0 as {}".format(x, idx) for idx, x in zip(pred_names, queries)]
        )
...
class GraftingLinearModel:
    ...
    def mask_model(self, top_k=None, alpha=None):
        ...
        if top_k is not None:
            ...
            raise Exception(
                "Could not get new model with top_k: {}, alpha: {}".format(top_k, alpha)
            )
